...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="/img/logoEducacionIcon.jpg">
    ...
    <link rel="stylesheet" href="https://unpkg.com/bootstrap-table@1.15.2/dist/extensions/sticky-header/bootstrap-table-sticky-header.css">
...
<body class="fixed-header dashboard menu-pin">
    ...
    <script>
            ...
            // }
            $(".select2").select2();
            let paginationContainer = $($("nav")[1]);

            paginationContainer.addClass("ml-auto");
            paginationContainer.css("width", "fit-content");
            paginationContainer.css("display", "flex");
            paginationContainer.css("margin-top", "33px");

            console.dir(paginationContainer);
            paginationContainer.find("a").each(function (index, element) {
                if ($(element).text().includes("Next »")) {
                    $(element).text("Siguiente »");
                }
                if ($(element).text().includes("« Previous")) {
                    $(element).text("« Anterior");
                }
            });

            paginationContainer.find("span").each(function (index, element) {
                if ($(element).text().includes("Next »")) {
                    $(element).text("Siguiente »");
                }
                if ($(element).text().includes("« Previous")) {
